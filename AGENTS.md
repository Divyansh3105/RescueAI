...
- The requirements, MVP scope, user flows and acceptance criteria are in **`docs/PRD.md`**. That file is the source of truth for *what* to build.
- Deadlines (department timeline, see `docs/PLAN.md`): Phase-I Examination Oct 26 – Nov 3, 2026; deployed prototype plus evaluation results by January 2027; Final Report April 2027; Phase-II Examination May 2027.
- `docs/PRD.md` is still a **draft with open assumptions** (see its last section). Do not treat an item marked `[Assumption]` as confirmed. Items marked `[Provisional]` are decided for now but must be reconsidered against scenario data; don't present them as final.

## Architecture Overview

The architecture is **partly built**: the three deployable pieces run and are live, but no feature behind them exists. Full details, diagrams and decision status are in **`docs/ARCHITECTURE.md`**. Read it before any structural change.
...
Anything still marked **Proposed** in `docs/ARCHITECTURE.md` (currently only AD11, the future Python ML service) needs the user's confirmation before you rely on it.
...
web/                   React SPA (Vite, Tailwind v4, shadcn/ui, TanStack Query)
  src/index.css        Design tokens from docs/DESIGN.md. Change a token here, never per component.
...
Integration tests against Docker PostgreSQL do not exist yet; their harness lands in Phase 2. See `docs/TESTING.md` for the strategy and the Definition of Done.

## Code Conventions

All coding rules are in **`docs/RULES.md`**: naming, strict TypeScript, layering, error handling, state management, git and the list of forbidden practices. Read it before writing code.

## Architecture Conventions

- **Build only MVP features F1–F14 from `docs/PRD.md`.** Anything under PRD "Future Features" or "Non-Goals" is out of scope unless the PRD is updated first.
...
The database is PostgreSQL, accessed through Drizzle ORM with drizzle-kit migrations. The entities and data-model rules are in `docs/ARCHITECTURE.md` → Database Architecture. The migration and audit rules are in `docs/RULES.md` → Database Rules.

## API Conventions

See `docs/ARCHITECTURE.md` → API Architecture and `docs/RULES.md` → API Rules.

## Security Requirements

These rules are decided in `docs/PRD.md`:
...
How these are implemented: `docs/ARCHITECTURE.md` → Security Architecture and `docs/RULES.md` → Security Rules.

## Dependency Guidelines

- Use npm only.
- Use the adopted tool for each job (see `docs/RULES.md` → Dependency Rules).
- Get the user's approval before adding a new framework, database or external service.

## Testing Requirements

- Each MVP feature is done only when the acceptance criteria it covers in `docs/PRD.md` (AC1–AC25) are verified.
- The required tests, the checks to run, and the Definition of Done are in **`docs/TESTING.md`**. Follow it before reporting any task complete.
...
4. The change follows `docs/RULES.md`, and follows `docs/DESIGN.md` if it touches UI.
5. If you added or changed a command, dependency, directory or convention, you updated this file (or `docs/RULES.md`) in the same change.
6. If you made a major architectural change (a new service, datastore, boundary, or change of stack), you updated `docs/ARCHITECTURE.md` in the same change. If you made or changed a significant decision, you added a `docs/DECISIONS.md` entry, superseding the old entry rather than rewriting it.
7. After a meaningful session, update `docs/MEMORY.md` (status, open problems, next steps) and remove anything in it that is out of date.
...
- Do not treat `[Assumption]` items in `docs/PRD.md`, or Proposed items in `docs/ARCHITECTURE.md`, as settled. Do not silently change them.
- Do not document commands, tools or conventions in this file that do not actually exist in the repo.
- Coding-level prohibitions are listed in `docs/RULES.md` → Forbidden Practices.

## Additional Documentation

- `docs/PRD.md` covers product scope, roles, features F1–F14, user flows, constraints, success metrics and acceptance criteria.
- `docs/DESIGN.md` covers colors, typography, layout, component patterns, states and accessibility (WCAG 2.2 AA). **Read it before creating or modifying any UI.**
- `docs/ARCHITECTURE.md` covers the stack, layers, data model, API areas, data flow, system boundaries, security implementation, deployment, and decision status (Decided or Proposed).
- `docs/RULES.md` covers the coding rules: organization, naming, types, errors, API, database, security, dependencies, state, testing, git, performance, and forbidden practices.
- `docs/MEMORY.md` covers the current state, known problems, next steps and session handoff. **Read it at the start of every session.**
- `docs/DECISIONS.md` is the decision log (D-001 onward): context, options, reasoning and trade-offs for each product and technical decision.
- `docs/TESTING.md` is the testing strategy: the stack, which layer needs which kind of test, mocking and test-data rules, required checks, and the Definition of Done.
- The project synopsis (`Synopsis_CSE27-364.pdf`) is the background research and full long-term vision. It is **not in this repository**. `docs/PRD.md` overrides it on scope.
